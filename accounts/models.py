...
class Committee(models.Model):
    ...
    members = models.ManyToManyField(
        settings.AUTH_USER_MODEL, related_name="committees", blank=True
    )
...
class User(AbstractUser):
    ...
    profile_picture = CloudinaryField(
        "image", null=True, blank=True, folder="profile_pictures"
    )
    membership_tier = models.CharField(
        max_length=20, choices=TIER_CHOICES, default="student", db_index=True
    )
    ...
    is_verified = models.BooleanField(
        default=False, db_index=True, help_text="Admin-verified member status"
    )
    ...
    def is_exco_or_trustee(self):
        """Quick check if user is executive (EXCO or Trustee)"""
...
# ============================================================================
# EXECUTIVE executive MODEL
# ============================================================================


class Executive(models.Model):
    """
    Executive executive profiles displayed on homepage.
    """
    ...
    position = models.CharField(
        max_length=100, help_text="e.g., National President, Vice President"
    )
    bio = models.TextField(blank=True, help_text="Short bio displayed on homepage")
    rank = models.PositiveIntegerField(
        default=1, help_text="Display order (1 for President, 2 for VP, etc.)"
    )
...
class StudentProfile(models.Model):
    ...
    user = models.OneToOneField(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="student_info"
    )
    ...
    matric_number = models.CharField(
        max_length=30, unique=True, help_text="University matriculation number"
    )
    level = models.IntegerField(
        choices=LEVEL_CHOICES, help_text="Current academic level"
    )
...
class BaseAnnouncement(models.Model):
    ...
    summary = models.TextField(blank=True, help_text="Short preview shown in listings")
    ...
    image = CloudinaryField("image", blank=True, null=True, folder="announcements")

    # Status fields
    is_published = models.BooleanField(
        default=True, db_index=True, help_text="Uncheck to hide this announcement"
    )
...
class Resource(models.Model):
    ...
    category = models.CharField(
        max_length=20, choices=CATEGORIES, default="general", db_index=True
    )
    file = models.FileField(upload_to="resources/")
    access_level = models.CharField(
        max_length=20, choices=ACCESS_LEVELS, default="public", db_index=True
    )
...
class CPDRecord(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="cpd_records"
    )
    activity_name = models.CharField(
        max_length=255, help_text="e.g., NAA Annual Conference 2026"
    )
    ...
    is_verified = models.BooleanField(
        default=False, db_index=True, help_text="Admin verification status"
    )
...
class CommitteeReport(models.Model):
    ...
    committee = models.ForeignKey(
        Committee, on_delete=models.CASCADE, related_name="reports"
    )
...
class Article(models.Model):
    ...
    content = CKEditor5Field(config_name="default")
    image = CloudinaryField("image", blank=True, null=True, folder="articles")
...
class EmailUpdate(models.Model):
    ...
    title = models.CharField(
        max_length=200, unique=True, help_text="Internal name for admins"
    )
    ...
    sendgrid_template_id = models.CharField(
        max_length=100, blank=True, null=True, help_text="SendGrid dynamic template ID"
    )
    is_active = models.BooleanField(
        default=True, help_text="Only one template can be active at a time"
    )
    created_at = models.DateTimeField(auto_now_add=True)
