...
from .models import (
    ...
    Resource, StudentProfile, StudentAnnouncement,
    CPDRecord, Notification, Article, Committee,
    CommitteeReport, CommitteeAnnouncement,
    send_verification_email,
)
from .forms import (
    NAAUserCreationForm, StudentProfileForm, ProfilePictureForm,
    CPDSubmissionForm, CommitteeReportForm, CommitteeAnnouncementForm,
    ArticleSubmissionForm, UserUpdateForm,
)
...
def register(request):
    """
    User registration with rate limiting (max 3 registrations per hour).
    """
    if request.user.is_authenticated:
        return redirect('home')
...
def home(request):
    ...
    announcements_list = Announcement.objects.filter(
        is_published=True
    ).select_related('author').order_by('-featured', '-date_posted')
    ...
    articles = Article.objects.filter(
        ...
        'id', 'title', 'slug', 'image', 'created_at', 'content'
    ...
    if not executives:
        executives = Executive.objects.filter(
            is_active=True
        ).select_related('user').order_by('rank')[:8]
...
def profile(request):
    """
    User profile page with multiple form handlers.
    Handles: profile picture, student profile, user info updates.
    """
    user = request.user
    
    # Handle profile picture upload
    if request.method == 'POST' and 'profile_picture' in request.FILES:
        p_form = ProfilePictureForm(request.POST, request.FILES, instance=user)
        if p_form.is_valid():
            p_form.save()
        ...
        p_form = ProfilePictureForm(instance=user)
    
    # Handle user info update (first name, last name)
    if request.method == 'POST' and 'update_info' in request.POST:
        u_form = UserUpdateForm(request.POST, instance=user)
        if u_form.is_valid():
            u_form.save()
            logger.info(f"Profile info updated for {user.username}")
            messages.success(request, "Profile information updated!")
            return redirect('profile')
    else:
        u_form = UserUpdateForm(instance=user)
    
    # Handle student profile (only for students)
    student_profile = None
    s_form = None
    
    if user.membership_tier == 'student':
        try:
            student_profile = user.student_info
        ...
        if request.method == 'POST' and 'matric_number' in request.POST:
            ...
            if s_form.is_valid():
                ...
                student_obj.save()
                logger.info(f"Student profile created/updated for {user.username}")
                messages.success(request, "Student details updated!")
                return redirect('profile')
        else:
            s_form = StudentProfileForm(
                instance=student_profile
            ) if student_profile else StudentProfileForm()
...
def download_constitution(request):
    ...
    if not request.user.is_verified:
        messages.warning(request, "Only verified members can download the constitution.")
        return redirect('profile')
...
def resource_library(request):
    ...
    resources = Resource.objects.filter(
        ...
        'id', 'title', 'category', 'file', 'description', 'uploaded_at'
    )
...
def cpd_tracker(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.info(f"CPD record created by {request.user.username}: {cpd.activity_name}")
...
def student_hub(request):
    ...
    student_announcements = StudentAnnouncement.objects.filter(
        Q(target_university='All') | Q(target_university=student_profile.university),
        is_published=True
    ).select_related('author').order_by('-date_posted')[:5]
    
    # Get university statistics
    stats = StudentProfile.objects.values('university').annotate(
        total=Count('id')
    ).order_by('-total')
    ...
    resources = Resource.objects.filter(
        category='academic',
        access_level__in=['public', 'student']
    ).select_related('uploaded_by').order_by('-uploaded_at')[:8]
...
def committee_dashboard(request, pk):
    ...
    context = {
        ...
        'announcements': committee.announcements.select_related(
            'author'
        ).order_by('-date_posted')[:10],
        'reports': committee.reports.select_related(
            'submitted_by'
...
def exco_verify_member(request, user_id):
    ...
    if member.is_verified:
        ...
        logger.info(
            f"{request.user.username} verified member {member.username}"
        )
...
class MemberListAPI(APIView):
    ...
    def get(self, request):
        ...
        if not request.user.is_verified:
            return Response(
                {"error": "Only verified members can access this API."},
                status=403
            )
        ...
        members = User.objects.filter(
            is_verified=True
        ).values('id', 'username', 'membership_tier')
...
class ExcoReportFetchAPI(APIView):
    ...
    def get(self, request):
        ...
        if not request.user.is_exco_or_trustee():
            return Response(
                {"error": "Unauthorized. EXCO access required."},
                status=403
            )
