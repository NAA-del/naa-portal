...
def committee_director_required(view_func):
    ...
    def wrapper(request, pk, *args, **kwargs):
        ...
        if not (is_director or is_exco):
            messages.error(request, "Access denied. You are not the director of this committee.")
            return redirect('profile')
...
def committee_member_required(view_func):
    ...
    def wrapper(request, pk, *args, **kwargs):
        ...
        if not (is_member or is_director or is_exco):
            messages.error(request, "Access denied. You are not a member of this committee.")
            return redirect('profile')
...
def verified_member_required(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if not request.user.is_verified:
            messages.warning(request, "This feature is only available to verified members.")
            return redirect('profile')
